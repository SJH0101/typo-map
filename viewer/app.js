...
function draw(){
  ...
  panels.forEach(function(P){
    ...
    for (const k in EMAP){
      ...
      // 여럿이 다 가진 선은 회색으로 물러나고, 한쪽만 가진 선이 색을 얻는다
      ctx.strokeStyle = (multi && !all) ? wcolor(P.who) : EDGE;
      ctx.lineWidth = (multi && !all ? 1.4 : 1) + mine.r*4.2;
      ctx.setLineDash(mine.sign<0 ? [7,4] : []);
