...
GROUP.update({n: '자리' for n in features.PLACE})
...
METRIC = {'색수': 'n_colors', '밝기': 'value', '최빈색': 'ground_share',
          ...
          '무게x': 'center_x', '무게y': 'center_y', '퍼짐x': 'spread_x',
          '퍼짐y': 'spread_y', '축공유': 'axis_share', '빈띠': 'band'}
...
NOTE = {
    ...
    '무게x': dict(뜻='글자 덩어리들의 무게중심이 판면 폭의 어디에 있는가',
        읽는법='0.5 면 가운데, 작으면 왼쪽으로 쏠린 판이다. 상자 넓이로 가중한다.',
        주의='색면과 그림은 세지 않고 «글자» 만 본다. 회전된 포스터는 빠진다.'),
    '무게y': dict(뜻='글자 덩어리들의 무게중심이 판면 높이의 어디에 있는가',
        읽는법='작으면 위쪽에 앉힌 판, 크면 아래로 내린 판이다.',
        주의='같은 이유로 그림은 빠진다. 큰 사진이 위를 채우고 글자가 아래 몰린 판은 «아래» 로 잡힌다.'),
    '퍼짐x': dict(뜻='덩어리 중심들이 가로로 얼마나 흩어졌나 (표준편차 / 판면 폭)',
        읽는법='작으면 한 세로줄에 모인 판, 크면 좌우로 벌린 판이다.',
        주의='덩어리가 둘뿐이면 값이 불안정하다.'),
    '퍼짐y': dict(뜻='덩어리 중심들이 세로로 얼마나 흩어졌나',
        읽는법='작으면 한 덩어리로 뭉친 판, 크면 위아래로 갈라 앉힌 판이다.',
        주의='같음.'),
    '축공유': dict(뜻='가장 많은 덩어리가 공유하는 왼쪽 정렬축의 비율',
        읽는법='1.0 이면 모든 덩어리가 한 축에 선다. 낮으면 축이 여럿이다.',
        주의='2px 안이면 같은 축으로 본다. 검출 상자가 흔들리면 축이 갈라져 보인다.'),
    '빈띠': dict(뜻='글자 영역 안에서 가장 넓은 «빈 가로 띠» 의 높이 / 판면 높이',
        읽는법='크면 판을 위아래로 뚜렷하게 가른 구성이다. 0 이면 틈 없이 이어진다.',
        주의='덩어리가 가로로 겹쳐 있으면 틈이 안 잡힌다. 세로 틈은 재지 않는다.'),
}
