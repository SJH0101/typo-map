...
(docs/loo_preregister.json).
    ...
    B 통념     **출처가 없다.** 내가 아는 대로 적은 값이다 (행간 1.4em ·
               마진 10% · 색 3). 「교과서 값」이라고 부르면 안 된다 —
               어느 책 몇 쪽인지 대지 못한다. 문헌으로 못 박기 전까지
               B 와의 비교는 결과로 쓸 수 없다.
...
CONV_LEAD_EM = 1.40  # 출처 없음. 「행간은 활자 크기의 1.2~1.5배」라는
                     # 말은 흔하지만 내가 어느 문헌에서도 확인하지 않았다.
CONV_MARGIN = 0.10   # 출처 없음
CONV_COLORS = 3      # 출처 없음
