...
한 줄을 여러 칸으로 적은 것은 v3 가 칸 나눔을 넣던 흔적이다. 원본은 탭으로
칸을 맞추지 않았다 — 넓은 띄어쓰기로 구분한 흐르는 글이다. 처음에 「낱말이
줄 안에서 축에 붙는다」고 읽은 것은 같은 앞말(leitung ) 뒤가 같은 x 에 온
착시였다 (docs/inline_test.json).
"""
