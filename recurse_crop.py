...
def build(path, det, box=(0.0, 0.0, 1.0, 1.0), id='r', depth=0, log=None, floor=None,
    ...
    # 한 단계에 «한 번만» 가른다. group() 은 크기·겹침·간격을 한꺼번에 보고
    # 최종 덩어리까지 붙이는 함수라 재귀에 안 맞았다 — Opernhaus 1966 에서
    # surya 84줄이 한 덩어리가 되고, 「자식이 하나면 멈춤」과 물려 트리가
    # 2마디에서 끝났다. 283장 중 75장이 그렇게 뭉갰다.
    kids = [DS.bbox(g) for g in DS.split_once(lines)]

    # 한 번 갈라도 하나면 더 쪼갤 것이 없다는 뜻이다. 내려가면 안 된다.
    # 루더 1955 에서 "Louis" 와 "Weber" 가 옳게 한 덩어리가 됐는데 또 내려갔고,
    # 살짝 다른 크롭에서 검출기가 조각 하나(39x15, "ouis")만 냈다.
    if len(kids) == 1:
        k = kids[0]
        n.kind = '글줄'
